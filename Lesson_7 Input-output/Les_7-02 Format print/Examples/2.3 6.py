"""
Дана последовательность вещественных чисел.
Определить среднее арифметическое среди кратных 7 элементов последовательности.

---------------
Тестовые значения:
[2.4, 6.2, 7.0, 14.0, 0.7, 12.4]    10.5
[2.3, 6.0, 7.0, 6.7, 21.0, 12.4, 77.0, 100.0, 16.9] 35
[14.0, 21.0, 700.0]     245
[2.4, 6.2, 7.7, 14.2, 0.7, 12.4]    no such numbers
"""

# l = [2.4, 6.2, 7.7, 14.2, 0.7, 12.4]
l = input('Введи последовательность вещественных чисел: ').split(',')
l = list(map(float, l))
# print(l)

i = 0
nums = []

while i<len(l):
    if l[i]%7 == 0:     # not l[i]%7
        nums.append(l[i])
    i += 1

if nums:
    res = sum(nums)/len(nums)
    print('Среднее арифметическое кратных 7 элементов последовательности {}: {:.2f}'.format(l, res))
else:
    print('Чисел кратных 7 в последовательности {} не найдено'.format(l))
